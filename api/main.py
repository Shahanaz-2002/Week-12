...
def debug_sample():
    return {
        "sample_query": "Knee pain | ACL injury | swelling",
        "sample_context": "Patient with chronic knee pain and swelling",
        "sample_fields": [
            "chief_complaint",
            "affected_body_part",
            "symptoms_duration",
...
def clinical_match(request: ClinicalMatchRequest):
    ...
    try:
        ...
        search_query = processed_inputs.get("search_query", "")
        generated_context = processed_inputs.get("generated_context", "")
        combined_symptoms = processed_inputs.get("combined_symptoms", "")
        ...
        if not str(search_query).strip():
            raise HTTPException(
                ...
                detail={
                    "status": "Failed",
                    "message": "No valid clinical input fields provided"
                }
            )
        ...
        final_response = {
            ...
            "patient_metadata": patient_metadata,
            "explanation": result.get("explanation", "Semantic clinical retrieval completed"),
            "warnings": [],
            "success": True
        }
        ...
        log_event(
            ...
            request_id,
            "Failed to complete clinical matching",
            {
                "error": str(e),
                "traceback": traceback.format_exc()
            }
        ...
        raise HTTPException(
            ...
            detail=standard_error_response(
                status_text="Failed",
                message="Clinical matching failed",
                request_id=request_id,
                error=str(e)
            )
        )
